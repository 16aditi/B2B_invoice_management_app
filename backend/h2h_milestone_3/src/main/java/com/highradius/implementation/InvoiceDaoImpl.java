...
public class InvoiceDaoImpl implements InvoiceDao{
	...
	public void insertInvoice(Invoice invoice) throws SQLException{
	    ...
	    try (Connection con = ConnectDb.connect();
	        ...
	        pstmt.setString(7, invoice.getAMOUNT_IN_USD());
	...
	public void deleteInvoice(String Sl_no) throws SQLException{
		try(Connection con = ConnectDb.connect();
				PreparedStatement pstmt = con.prepareStatement(DELETE);){
			pstmt.setString(1,Sl_no);
			pstmt.executeUpdate();
		}catch(Exception e) {
			e.printStackTrace();
		}
	...
	public Invoice getInvoiceId(String CUSTOMER_ORDER_ID) {
        ...
        try (Connection con = ConnectDb.connect();
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    invoice = new Invoice();
                    invoice.setSl_no(rs.getString("Sl_no"));
